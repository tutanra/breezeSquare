...
namespace BreezeSquare
{
    ...
    QColor Button::backgroundColor() const
    {
        ...
        QColor greenColor = KColorScheme(QPalette::Active, KColorScheme::Window).foreground(KColorScheme::PositiveText).color();
        QColor yellowColor = KColorScheme(QPalette::Active, KColorScheme::Window).foreground(KColorScheme::NeutralText).color();
        QColor greyColor = KColorUtils::mix(d->fontColor(), d->titleBarColor(), 0.9);

        if (isPressed())
        {

            if (type() == DecorationButtonType::Close)
                return KColorUtils::darken(redColor, 0.4);
            else
        ...
        else if (isHovered())
        {
            ...
            if (type() == DecorationButtonType::Close)
                return c->isActive() ? KColorUtils::darken(redColor, 0.2) : redColor;
        ...
        else if (d->internalSettings()->outlineCloseButton() && type() == DecorationButtonType::Close)
        {
